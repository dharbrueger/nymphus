...
export class MessagesService {
  ...
  public async sendSubscriberWaterReminderToChannel(
    channel: Channel,
    guild: Guild | undefined,
    subscriberList: Reminder[]
  ): Promise<void> {
    if (!guild) {
    ...
    const message = await this.appendSubscriberMessages(
      guild,
      this.defaultReminderMessage
    );
    this.sendMessageToTextChannel(channel, message);
  }
  ...
  private async appendSubscriberMessages(
    guild: Guild,
    message: string
  ): Promise<string> {
    const subscriberList = await ReminderService.getReminders({
      guildId: guild.id,
    });

    const memberMessages = await Promise.all(
