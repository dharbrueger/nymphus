...
export const Enable: Command = {
  ...
  run: async (client: Client, interaction: CommandInteraction) => {
    ...
    try {
      await NotificationChannelService.addNotificationChannel({
        guildId,
        channelId,
      });

      await interaction.followUp({
        content: 'This channel will now receive water reminders! 💧',
        ephemeral: true,
      });
    } catch (error) {
      ...
      const errorMessage = duplicateError
        ? 'This channel is already subscribed to water reminders.'
        : 'There was an error subscribing this channel to water reminders. Please try again later.';
